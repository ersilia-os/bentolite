...
from .utils import decompress_gzip_request
from ..types import AwsLambdaEvent, FileLike, HTTPRequest, InferenceTask

ApiFuncArgs = Tuple[Sequence[str],]


def detect_encoding(bytes_):
    if chardet is not None:
        return chardet.detect(bytes_)["encoding"] or "utf-8"
    ...
    # If no encoding worked, fallback to utf-8
    return "utf-8"
...
class StringInput(BaseInputAdapter):
    ...
    def from_http_request(self, req: HTTPRequest) -> InferenceTask[str]:
        if req.headers.content_type == "multipart/form-data":
            ...
            charset = detect_encoding(bytes_)
        else:
            bytes_ = req.body
